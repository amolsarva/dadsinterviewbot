...
export default async function handler(req) {
  ...
  try {
    ...
    const { url: audioUrl } = await put(`${base}.webm`, audioFile, { access: 'public', addRandomSuffix: false, contentType: audioFile.type || 'audio/webm' })
    ...
    if (SENDGRID_API_KEY && FROM_EMAIL) {
      await sendEmail({
        ...
        html: `<h2>Your AI Interview is Ready</h2>
          <p><b>ID:</b> ${id}</p>
          <p><b>Date:</b> ${new Date(timestamp).toLocaleString()}</p>
          <ul><li><a href="${audioUrl}">Audio (.webm)</a></li><li><a href="${transcriptUrl}">Transcript (.txt)</a></li></ul>`,
      })
    }

    return new Response(JSON.stringify({ ok:true, id, audioUrl, transcriptUrl }), { status:200, headers:{ 'Content-Type':'application/json' } })
  } catch (err) {
