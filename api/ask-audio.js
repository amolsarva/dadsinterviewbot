...
export default async function handler(req, res){
  try{
    ...
    if (!process.env.GOOGLE_API_KEY){
      return res.status(200).json({ ok:true, provider, reply:"Who was with you in that room—name one person and what they wore.", transcript:text||"", end_intent:false })
    }
    ...
    return res.status(200).json({ ok:true, provider: process.env.PROVIDER || 'google', reply:"Who else was there with you—first name and one detail about them?", transcript:"", end_intent:false })
  }
}
