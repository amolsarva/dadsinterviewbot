...
export default async function handler(req){
  ...
  const path = `sessions/${sessionId}/session-${sessionId}.json`
  const manifestUrl = await saveBlob(path, Buffer.from(JSON.stringify(manifest,null,2)), { contentType:'application/json' })
