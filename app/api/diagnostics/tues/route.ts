...
export async function POST() {
  ...
  try {
    stage = 'collect-env'
    logInfo('Starting Netlify-hosted TUES credential diagnostic', { stage })
    ...
    const authMethod = (readOptionalEnv('TUES_AUTH_METHOD') ?? 'POST').toUpperCase()
    if (authMethod !== 'POST' && authMethod !== 'GET') {
      throw new Error(`Unsupported TUES_AUTH_METHOD ${authMethod}. Only POST and GET are supported.`)
    }

    const verifyMethod = envValues.TUES_VERIFY_METHOD.toUpperCase()
    const allowedVerifyMethods = ['GET', 'POST', 'PUT', 'PATCH', 'DELETE']
    if (!allowedVerifyMethods.includes(verifyMethod)) {
      throw new Error(`TUES_VERIFY_METHOD ${verifyMethod} is not supported. Use one of ${allowedVerifyMethods.join(', ')}.`)
    }
    ...
    const expectedStatus = Number.parseInt(envValues.TUES_EXPECTED_STATUS, 10)
    if (!Number.isFinite(expectedStatus)) {
      throw new Error(`TUES_EXPECTED_STATUS must be a valid integer. Received ${envValues.TUES_EXPECTED_STATUS}`)
    }

    logInfo('Collected environment configuration', {
      stage,
