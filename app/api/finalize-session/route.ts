...
import { putBlobFromBuffer, listBlobs } from '@/lib/blob'
import { sendSummaryEmail } from '@/lib/email'
import { getSession, rememberSessionManifest } from '@/lib/data'
import { z } from 'zod'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const transcriptTxtBlob = await putBlobFromBuffer(
      `sessions/${sessionId}/transcript-${sessionId}.txt`,
      Buffer.from(transcriptText, 'utf8'),
      'text/plain; charset=utf-8'
    )
    const transcriptTxtUrl = transcriptTxtBlob.downloadUrl || transcriptTxtBlob.url
    const transcriptJsonBlob = await putBlobFromBuffer(
      `sessions/${sessionId}/transcript-${sessionId}.json`,
      Buffer.from(JSON.stringify(transcriptJson, null, 2), 'utf8'),
      'application/json'
    )
    const transcriptJsonUrl = transcriptJsonBlob.downloadUrl || transcriptJsonBlob.url
    ...
    const manifestForMemory = {
      ...manifest,
      artifacts: { ...manifest.artifacts, session_manifest: manifestUrl },
    }
