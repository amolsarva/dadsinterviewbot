...
async function buildMemoryPrompt(userId: string, sessionId: string | undefined): Promise<MemoryPrompt> {
  if (!sessionId) {
    return {
  ...
  const primer = await getMemoryPrimer(userId)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const memory = await buildMemoryPrompt(userId, sessionId)
